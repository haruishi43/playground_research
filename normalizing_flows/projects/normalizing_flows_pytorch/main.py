...
class Model(object):
    def __init__(self, dims=(2,), datatype=None, cfg=None):
        ...
        self.net = networks[self.name](
            dims=self.dims, datatype=datatype, cfg=cfg.network
        )
        ...
        if cfg.optimizer.name == "rmsprop":
            ...
            raise Exception(
                'optimizer "%s" is currently not supported' % (cfg.optimizer.name)
            )

        self.schduler = torch.optim.lr_scheduler.StepLR(
            self.optim,
...
def main(cfg):
    ...
    for data in dataset:
        ...
        if step == start_step + 1 or step % (cfg.run.display * 100) == 0:
            writer.add_scalar(
                "{:s}/train/loss".format(dataset.dtype), loss.item(), step
            )
