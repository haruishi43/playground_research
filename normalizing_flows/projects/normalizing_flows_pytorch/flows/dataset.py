...
class FlowDataLoader(object):
    ...
    def _initialize(self):
        ...
        if self.name == "mnist":
            ...
            self.dset = torchvision.datasets.CIFAR10(
                root=os.path.join(data_root, "cifar10"), train=True, download=True
            )
            self.dims = (3, 32, 32)
            self.dtype = "image"
    ...
    def __iter__(self):
        for _ in range(self.total_steps):
            ...
            if self.dtype == "image":
                data = [
                    np.asarray(self.dset[i][0], dtype="float32") / 255.0 for i in idx
                ]
                data = np.reshape(
                    data, (self.batch_size, self.dims[1], self.dims[2], -1)
                )
