...
class BatchNorm(nn.Module):
    ...
    def forward(self, x, log_det_jacob):
        if self.training:
            ...
            x_var = (
                torch.mean((x_reshape - x_mean).pow(2), dim=[0, 2], keepdim=True)
                + self.eps
            )
...
class ResBlockLinear(nn.Module):
    def __init__(self, in_channels, out_channels, weight_norm=True):
        ...
        if in_channels != out_channels:
            self.bridge = weight_norm_wrapper(
                nn.Linear(in_channels, out_channels), weight_norm
            )
        else:
...
class ResBlock2d(nn.Module):
    def __init__(self, in_channels, out_channels, weight_norm=True):
        ...
        self.net = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            weight_norm_wrapper(
                nn.Conv2d(in_channels, out_channels, 3, 1, 1), weight_norm
            ),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            weight_norm_wrapper(
                nn.Conv2d(out_channels, out_channels, 3, 1, 1), weight_norm
            ),
        )
...
class MLP(nn.Module):
    def __init__(
        self, in_channels, out_channels, base_filters=32, n_blocks=2, weight_norm=True
    ):
...
class ConvNet(nn.Module):
    def __init__(
        ...
        self.in_block = nn.Sequential(
            weight_norm_wrapper(
                nn.Conv2d(in_channels, base_filters, 3, 1, 1), weight_norm
            ),
        )
        ...
        self.out_block = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            weight_norm_wrapper(
                nn.Conv2d(base_filters, out_channels, 1, 1, 0), weight_norm
            ),
        )
...
class InvertibleConv1x1(nn.Module):
    ...
    def __init__(self, in_out_channels):
        ...
        L_mask = np.tril(
            np.ones((in_out_channels, in_out_channels), dtype="float32"), k=-1
        )
    ...
    def backward(self, y, log_df_dz):
        with torch.no_grad():
            LU = (
                self.L * self.L_mask
                + self.U * self.U_mask
                + torch.diag(self.sign_s * torch.exp(self.log_s))
            )

            y_reshape = y.view(y.size(0), y.size(1), -1)
            y_reshape = torch.lu_solve(
                y_reshape, LU.unsqueeze(0), self.pivots.unsqueeze(0)
            )
...
class GatedAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        params = self.conv1(x_reshape).view(
            B, 3 * self.heads, D, -1
        )  # (B, 3head, D, *)
