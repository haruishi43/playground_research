...
def log_df_dz_fixed(g, z, n_samples=1, n_power_series=8):
    ...
    w_t_J_fn = lambda w, z=z, g=g: torch.autograd.grad(
        g, z, grad_outputs=w, retain_graph=True, create_graph=True
    )[0]
    ...
    for j in range(n_samples):
        ...
        for k in range(1, n_power_series + 1):
            w = torch.autograd.grad(
                g, z, w, create_graph=is_training, retain_graph=True
            )[0]
...
class MemorySavedLogDetEstimator(torch.autograd.Function):
    ...
    def forward(ctx, logdet_fn, x, g_fn, training, *g_params):
        ...
        with torch.enable_grad():
            ...
            if ctx.training:
                dlogdetJg_dx, *dlogdetJg_dtheta = torch.autograd.grad(
                    logdetJg.sum(), [x] + theta, retain_graph=True, allow_unused=True
                )
    ...
    def backward(ctx, dL_dg, dL_dlogdetJg):
        ...
        with torch.enable_grad():
            ...
            dL_dx_1st, *dL_dtheta_1st = torch.autograd.grad(
                g, [x] + theta, grad_outputs=dL_dg, allow_unused=True
            )
        ...
        with torch.no_grad():
            ...
            dL_dtheta_2nd = tuple(
                [
                    g * dL_dlogdetJg_scalar if g is not None else None
                    for g in dlogdetJg_dtheta
                ]
            )
        ...
        with torch.no_grad():
            ...
            dL_dtheta = tuple(
                [
                    g1 + g2 if g2 is not None else g1
                    for g1, g2 in zip(dL_dtheta_1st, dL_dtheta_2nd)
                ]
            )
    ...
    def _get_logdet_estimator(self):
        if self.training:
            ...
            logdet_fn = lambda g, z: log_df_dz_unbias(
                g, z, 1, is_training=self.training
            )
        else:
            if self.estimator == "exact":
                logdet_fn = log_df_dz_exact
            elif self.estimator == "fixed":
                logdet_fn = lambda g, z: log_df_dz_fixed(
                    g, z, n_samples=4, n_power_series=8
                )
...
class InvertibleResConv2d(InvertibleResBlockBase):
    def __init__(
        ...
        for i, (in_dims, out_dims) in enumerate(zip(hidden_dims[:-1], hidden_dims[1:])):
            layers.append(
                spectral_norm(nn.Conv2d(in_dims, out_dims, 3, 1, 1), coeff=self.coeff)
            )
            if i != len(hidden_dims) - 2:
                layers.append(act_fn())
