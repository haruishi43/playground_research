...
def trace_df_dz_exact(f, z):
    ...
    n_dims = z.size(1)
    diags = [
        torch.autograd.grad(f[:, i].sum(), z, create_graph=True, retain_graph=True)[0][
            :, i
        ]
...
def trace_df_dz_hutchinson(f, z, n_samples=1, is_training=True):
    ...
    w_t_J_fn = lambda w, z=z, f=f: torch.autograd.grad(
        f, z, grad_outputs=w, retain_graph=True, create_graph=is_training
    )[0]
...
class ConcatConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0):
        ...
        self.conv = nn.Conv2d(
            in_channels + 1, out_channels, kernel_size, stride, padding
        )
...
class CNF(nn.Module):
    ...
    def __init__(
        ...
        assert backprop in ["normal", "adjoint"], 'unsupported backprop type "%s"' % (
            backprop
        )
