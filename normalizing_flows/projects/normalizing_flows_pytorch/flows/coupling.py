...
class AbstractCoupling(nn.Module):
    ...
    def __init__(self, dims, masking="checkerboard", odd=False):
        ...
        if len(dims) == 1:
            ...
            self.unsqueeze = lambda z0, z1, odd=odd: channel_merge(
                z0, z1, dim=1, odd=odd
            )
        else:
            raise Exception(
                "unsupported combination of masking and dimension: %s, %s"
                % (masking, str(dims))
            )
...
class MixLogAttnCoupling(AbstractCoupling):
    ...
    def __init__(
        self, dims, masking="checkerboard", odd=False, base_filters=32, n_mixtures=4
    ):
        ...
        if len(dims) == 1:
            ...
            if masking == "checkerboard":
                ...
                self.sections = [dims[0] // 2] * 2 + [
                    dims[0] // 2 * self.n_mixtures
                ] * 3
