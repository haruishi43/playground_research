...
def parse_args():
    ...
    if args.output_dir == "" and not args.show:
        raise ValueError(
            "if '--output-dir' and '--show' are not set, "
...
def build_dataset_pipelines(cfg, phase):
    ...
    loadimage_pipeline = []
    if (
        len(data_cfg.pipeline) != 0
        and data_cfg.pipeline[0]["type"] == "LoadImageFromFile"
    ):
...
def prepare_imgs(args, imgs: List[np.ndarray], steps=None):
    ...
    if args.adaptive:
        for i, img in enumerate(imgs):
            imgs[i] = adaptive_size(
                img, args.min_edge_length, args.max_edge_length
            )
    else:
...
def get_display_img(args, item, pipelines):
    ...
    if args.mode == "original":
        ...
        image = prepare_imgs(
            args, [pipeline_images[0], pipeline_images[-1]], steps
        )
...
def main():
    ...
    cfg = retrieve_data_cfg(
        args.config, args.skip_type, args.cfg_options, args.phase
    )
