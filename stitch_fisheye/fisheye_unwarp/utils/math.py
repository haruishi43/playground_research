...
def rotation_matrix_decompose(r):
    """This function helps analyse rotation matrix,
    by decompose it into rotation on x,y,z axis.
    Please note the eular angles provided is rotated
    in z, y, x order. See: http://nghiaho.com/?page_id=846.

    Args:
        r: Rotation matrix.

    Returns:
        x: Rotation on x axis, in rads
        y: Rotation on y axis, in rads
        z: Rotation on z axis, in rads
    """
    return np.array(
        (
            math.atan2(r[2][1], r[2][2]),
            math.atan2(
                -r[2][0], math.sqrt(r[2][1] * r[2][1] + r[2][2] * r[2][2])
            ),
...
def solve_quadratic(a, b, c):
    """This function solves quadratic, ax^2 + bx + c = 0.
    Please note it only gives real solution(s).
    ...
    Returns:
        x1,x2: Two solutions, x1 will always be larger.
            if there's no solution it will return None
    """
    if b * b - 4 * a * c < 0:
...
def find_rigid_transformation(A, B):
    """This function finds optimal rigid transformation
    on point set A and point set B. Please note you should use matrix
    rather than array. Result should satisfy R*A + t = B.

    Args:
        A: A Nxm numpy **matrix**. N is the count of sample points,
